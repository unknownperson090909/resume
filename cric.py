...
async def scorecard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """🏏 Detailed scorecard with strike rates and boundaries"""
...
async def players_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Legend
    text += "\n━━━━━━━━━━━━━━━━━━━━━━\n"
    text += "<i>👑 Captain | 🪙 Toss Winner\n"
    text += "🏏 Batting | 🔄 Non-striker\n"
    text += "⚡ Bowling | ❌ Out</i>\n"
    text += "━━━━━━━━━━━━━━━━━━━━━━"
...
async def mystats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """🏏 Complete cricket stats with proper tables"""
    user_id = update.effective_user.id
    ...
    if not row or row[0] == 0:
        await update.message.reply_text(
            "🏏 No stats yet! Play some matches to build your cricket profile.",
    ...
    text = f"🏏 {user_name}'s Cricket Card\n\n"
    text += "📊 MATCH RECORD\n"
    text += f"```\n"
    text += f"Played    : {matches}\n"
    text += f"Won       : {won}\n"
    ...
    text += f"Balls     : {balls}\n"
    text += f"Average   : {avg}\n"
    text += f"Strike Rate: {sr}\n"
    text += f"High Score: {high_score}\n"
    text += f"```\n"
    text += "🎯 MILESTONES\n"
    text += f"```\n"
    text += f"Hundreds  : {hundreds}\n"
    text += f"Fifties   : {fifties}\n"
    text += f"Ducks     : {ducks}\n"
    text += f"```\n"
    text += "💥 BOUNDARIES\n"
    text += f"```\n"
    ...
    text += "⚾ BOWLING\n"
    text += f"```\n"
    text += f"Wickets   : {wickets}\n"
    text += f"Best      : {best_bowl}\n"
    text += f"```"
    
    try:
        if MYSTATS_IMAGE_FILE_ID and MYSTATS_IMAGE_FILE_ID != "YOUR_MYSTATS_IMAGE_FILE_ID_HERE":
            await update.message.reply_photo(photo=MYSTATS_IMAGE_FILE_ID, caption=text, parse_mode=ParseMode.MARKDOWN)
        else:
...
async def botstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT COUNT(DISTINCT user_id) FROM player_stats')
    total_users = c.fetchone()[0]
    c.execute('SELECT SUM(matches_played) FROM player_stats')
    total_matches = c.fetchone()[0] or 0
    c.execute('SELECT SUM(total_runs) FROM player_stats')
    total_runs = c.fetchone()[0] or 0
    c.execute('SELECT SUM(total_wickets) FROM player_stats')
    total_wickets = c.fetchone()[0] or 0
    c.execute('SELECT SUM(total_sixes) FROM player_stats')
    ...
    text += f"Active Now: {active_match_count}\n"
    text += f"```\n"
    text += "🏏 CRICKET STATS\n"
    text += f"```\n"
    text += f"Matches   : {total_matches}\n"
    text += f"Runs      : {total_runs:,}\n"
    text += f"Wickets   : {total_wickets}\n"
    text += f"Sixes     : {total_sixes}\n"
    text += f"```\n"
    text += "🎯 ACTIVE NOW\n"
    text += f"```\n"
    text += f"Matches   : {active_match_count}\n"
    text += f"Auctions  : {active_auction_count}\n"
    text += f"```\n"
...
async def reg_price_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(group_id, f"🏏 New registration!\n{full_name} joined (Base: {base_price} coins)")
        except:
            pass
    except:
